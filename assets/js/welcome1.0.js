// 进行 fetch 请求
fetch('https://api.76.al/api/ip/query?key=ObKwGuq4BXztlCRrQB6SyksJtz') // 申请地址：https://api.76.al/
    .then(response => {
        if (!response.ok) {
...
function showWelcome() {
    ...
    let dist = getDistance(118.839900, 31.952600, ipLocation.data.lng, ipLocation.data.lat); // 修改自己的经度（121.413921）纬度（31.089290）
    ...
    if (welcomeInfoElement) {
        welcomeInfoElement.innerHTML =
            `欢迎来自 <b><span style="color: var(--kouseki-ip-color);font-size: var(--kouseki-gl-size)">${pos}</span></b> 的小友💖<br>${posdesc}🍂<br>当前位置距博主约 <b><span style="color: var(--kouseki-ip-color)">${dist}</span></b> 公里！<br>您的IP地址为：<b><span style="color: var(--kouseki-ip-color)">${ip}</span></b><br>${timeChange} <br>`;
    } else {
